...
// Settings
int sysid = 255;             ///< The unique system id of this MAV, 0-127. Has to be consistent across the system
int compid = 0;
std::string port = "/dev/ttyUSB0";              ///< The serial port name, e.g. /dev/ttyUSB0
//std::string port = "/dev/pts/18";              ///< The serial port name, e.g. /dev/ttyUSB0
...
//	  std::cout << "Heartbeat type: " <<  (int)heartbeat.type << "\t" << " sysid: " << (int)message.sysid << std::endl;
	  current_base_mode = heartbeat.base_mode;

	  if(current_base_mode & MAV_MODE_FLAG_SAFETY_ARMED){
...
void mavlinkCallback(const mavlink::Mavlink &mavlink_msg)
{

  /**
   * Convert mavlink::Mavlink to mavlink_message_t
   */
  mavlink_message_t msg;
  msg.msgid = mavlink_msg.msgid;

  static uint8_t mavlink_crcs[] = MAVLINK_MESSAGE_CRCS;

  //Copy payload from mavlink_msg (from ROS) to the new "real" mavlink message
  copy(mavlink_msg.payload64.begin(), mavlink_msg.payload64.end(), msg.payload64);

  mavlink_finalize_message_chan(&msg, mavlink_msg.sysid, mavlink_msg.compid, MAVLINK_COMM_0,
                                mavlink_msg.len, mavlink_crcs[msg.msgid]);

  /**
   * Send mavlink_message to UART
   */
  if (verbose)
    ROS_INFO("Sent Mavlink from ROS to UART, Message-ID: [%i]", mavlink_msg.msgid);
...
//        uint8_t buf[MAVLINK_MAX_PACKET_LEN];
        ...
        uint16_t r = 0;
        uint16_t buttons = 0;
        uint8_t target = MAV_TYPE_QUADROTOR;

        // Pack the message
//      mavlink_msg_heartbeat_pack(mavlink_system.sysid, mavlink_system.compid, &msg, system_type, autopilot_type, system_mode, custom_mode, system_state);
        mavlink_msg_manual_control_pack(mavlink_system.sysid, mavlink_system.compid, &msg_tmp, system_type, x, y, z, r, buttons);
...
///////////////////////////////////////////////////////////////////////////
  ...
  int messageLength = mavlink_msg_to_send_buffer(buffer, &msg_tmp);
  if (debug)
    printf("Writing %d bytes\n", messageLength);
  int written = write(fd, (char*)buffer, messageLength);
  tcflush(fd, TCOFLUSH);
  std::cout << "Sending RC" << std::endl;
...
void heartbeatCallback(const ros::TimerEvent& timeparm){
//	ROS_INFO("Send: rc override");
	static int count = 0;

	ROS_INFO("Send: Heartbeat");
	...
	mavlink_msg_heartbeat_pack(sysid, compid, &msg, MAV_TYPE_GCS, MAV_AUTOPILOT_INVALID, 0, 0, 0);

	int messageLength = mavlink_msg_to_send_buffer(buf, &msg);
	int written = write(fd, (char*)buf, messageLength);
	tcflush(fd, TCOFLUSH);
	if (messageLength != written)
		fprintf(stderr, "ERROR: Wrote %d bytes but should have written %d\n", written, messageLength);
	...
	if(++count == 2) {
		ROS_INFO("Arming..");
		mavlink_message_t msg_long;
		uint8_t buf_long[MAVLINK_MAX_PACKET_LEN];



		mavlink_msg_command_long_pack(sysid, compid,  &msg_long, 1, MAV_COMP_ID_SYSTEM_CONTROL, MAV_CMD_COMPONENT_ARM_DISARM, 0,1, 0, 0, 0, 0, 0, 0);
		int messageLength = mavlink_msg_to_send_buffer(buf_long, &msg_long);
	        int written = write(fd, (char*)buf_long, messageLength);
		tcflush(fd, TCOFLUSH);
		if (messageLength != written)
			fprintf(stderr, "ERROR: Wrote %d bytes but should have written %d\n", written, messageLength);
	}
...
/*
		...
		mavlink_msg_command_long_pack(10, 0,  &msg_long, 1, MAV_COMP_ID_ALL, MAV_CMD_DO_CHANGE_SPEED, 0,1, -1, 70, 0, 0, 0, 0);
		int messageLength = mavlink_msg_to_send_buffer(buf_long, &msg_long);
	        int written = write(fd, (char*)buf_long, messageLength);
		tcflush(fd, TCOFLUSH);
		if (messageLength != written)
...
//		mavlink_msg_rc_channels_override_pack(10, 0, &msg_long, 1,MAV_COMP_ID_SYSTEM_CONTROL, 1000,1000,15100,1000,1000,1000,1000,1000);
		mavlink_msg_rc_channels_override_pack(sysid, compid, &msg_long, 1,0, 1000,1000,1500,1000,1000,1000,1000,1000);
...
{
  ros::init(argc, argv, "mavlink_serial");
  ...
  mavlink_pub = mavlink_nh.advertise<mavlink::Mavlink>("from", 1000);
  ...
  //Publish heartbeat with 1 hz
  ros::Timer timer = nh.createTimer(ros::Duration(1), heartbeatCallback);
